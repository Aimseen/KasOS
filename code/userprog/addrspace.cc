...
static void ReadAtVirtual(OpenFile *executable, int virtualaddr,int numBytes, int position,TranslationEntry *pageTable,unsigned numPages){
  char * into=(char*)malloc(sizeof(char)*numBytes);
  executable->ReadAt(into,numBytes,position);
  TranslationEntry * memp=machine->pageTable;
  unsigned int memi=machine->pageTableSize;
  machine->pageTable=pageTable;
  machine->pageTableSize=numPages;
  for(int i=0;i<numBytes;i++){
    machine->WriteMem(virtualaddr+i,1,into[i]);
  }
  machine->pageTable=memp;
  machine->pageTableSize=memi;
}
